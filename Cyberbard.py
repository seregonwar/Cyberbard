...
# Check if the request was successful.
 if response.status_code == 200:
  ...
  if identifier := soup.find('input', {'id': 'identifierId'}):
   email = input("Enter your Google email address: ")
   identifier['value'] = email

   if next_button := soup.find('div', {'id': 'identifierNext'}):
    ...
    if response.status_code == 200:
     ...
     if password_field := soup.find('input', {'name': 'password'}):
      password_field['value'] = password

      if submit_button := soup.find('div', {'id': 'passwordNext'}):
